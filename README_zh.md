...
- **`KBUILD_BUILD_TIMESTAMP`** 必定设置：`build_time` 留空时取锁定内核 commit 的日期，因此同一个 lock 的两次构建带有相同的时间戳与产物名
...
- 已验证：每个 profile 都从同一个 lock 连编两次，Image 与 AK3 均逐字节一致；九个 profile 也全部在 CI runner 上构建过，产物与本机逐字节相同（证据 `docs/evidence/t25-reproducibility-and-cross-host-20260919.json`）。一个 ReSukiSU + SUSFS Inline 候选另外通过了部分真机验收。完整功能验收仍未宣称。
